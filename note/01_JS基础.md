...
- 每个class/函数都有一个显示原型: `prototype`
  ...
  - 给函数对象添加prototype属性, 属性值指定为空的Object实例对象   {}
  - 给原型对象添加constructor属性, 值为函数

- 每个实例都有一个隐式原型: `__proto__`

  - 实例对象创建后会自动执行: `this.__proto__ = Fn.prototype`

- 实例的`__proto__`与对应class的`prototype`都指向原型对象
...
- 使用闭包: 执行内部函数
- 释放闭包: 让内部函数对象成为垃圾对象, 断开指向它的引用
- 闭包的应用: 举删除删除列表中的的某个商品的例子(带确定框)
...
- 界面第一次渲染: 初始化同步代码 ==> 所有的微任务==> 渲染界面==> 执行第一个宏任务
  ...
  总结: 清空微队列中所有微任务 ==> 渲染界(GUI线程) ==> 执行宏队列中的第一个宏任务
